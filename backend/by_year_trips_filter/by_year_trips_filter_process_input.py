...
def by_year_trips_filter_process_input(message_type: bytes, message_body: bytes, client_id):
    if message_type == common.network.constants.TRIPS_BATCH:
        ...
        if len(filtered_trips) == 0:
            ...
            return common.network.constants.TRIPS_BATCH + client_id.encode() + pickle.dumps((filtered_trips, city))
