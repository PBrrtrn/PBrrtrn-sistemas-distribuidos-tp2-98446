...
class ClientDataIngestor:
    def __init__(self, wrapped_socket: SocketWrapper,
        ...
        self.client_id_in_bytes = client_id.encode()
    ...
    def generate_next_message_id(self):
        current_id_in_bytes = str(self.current_message_id).zfill(common.network.constants.MESSAGE_ID_LEN).encode()
        self.current_message_id += 1
        return current_id_in_bytes
    ...
    def receive_and_handle_stations_batch(self, wrapped_socket):
        ...
        while True:
            ...
            if message_type == common.network.constants.STATIONS_END:
                self.stations_exchange_writer.write(common.network.constants.STATIONS_END + self.client_id_in_bytes
                                                    + self.generate_next_message_id() + city.encode('utf-8'),
            ...
            message = common.network.constants.STATIONS_BATCH + self.client_id_in_bytes + \
                      self.generate_next_message_id() + pickle.dumps((raw_batch, city))
    ...
    def receive_and_handle_weather_batch(self, wrapped_socket):
        ...
        while True:
            ...
            message = common.network.constants.WEATHER_BATCH + self.client_id_in_bytes + \
                      self.generate_next_message_id() + pickle.dumps((raw_batch, city))
    ...
    def receive_and_handle_trips_batch(self, wrapped_socket):
        ...
        while True:
            ...
            message = common.network.constants.TRIPS_BATCH + self.client_id_in_bytes + \
                      self.generate_next_message_id() + pickle.dumps((raw_batch, city))
    ...
    def execute_queries(self, wrapped_socket):
        ...
        response = self.montreal_stations_over_6km_avg_trip_distance_rpc.call(
            common.network.constants.EXECUTE_QUERIES + self.client_id_in_bytes + self.generate_next_message_id()
        )

        wrapped_socket.send(common.network.constants.MONTREAL_STATIONS_OVER_6KM_AVG_TRIP_DISTANCE_RESULT +
                            self.client_id_in_bytes +
                            len(response[common.network.constants.MESSAGE_HEADER_LEN:]).to_bytes(4, 'big') +
                            response[common.network.constants.MESSAGE_HEADER_LEN:])
        ...
        wrapped_socket.send(common.network.constants.WITH_PRECIPITATIONS_AVG_TRIP_DURATION_RESULT +
                            self.client_id_in_bytes +
                            len(raw_avg_duration_response[common.network.constants.MESSAGE_HEADER_LEN:]).to_bytes(4, 'big') +
                            raw_avg_duration_response[common.network.constants.MESSAGE_HEADER_LEN:])
        ...
        wrapped_socket.send(common.network.constants.DOUBLED_YEARLY_TRIPS_STATION_NAMES_RESULT +
                            self.client_id_in_bytes +
                            len(raw_doubled_station_names_response[common.network.constants.MESSAGE_HEADER_LEN:]).to_bytes(4, 'big') +
                            raw_doubled_station_names_response[common.network.constants.MESSAGE_HEADER_LEN:])
        ...
        self.doubled_yearly_trips_stations_rpc.write_eof(common.network.constants.END_QUERY +
                                                         self.client_id_in_bytes + self.generate_next_message_id())
